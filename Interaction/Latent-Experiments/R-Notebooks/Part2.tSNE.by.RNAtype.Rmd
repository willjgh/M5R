...
```{r}
rna<-as.matrix(read.delim(file = "C:/Users/willi/Documents/ProjectPaper/M5R/Interaction/Latent-Experiments/Data/GSE151334_counts.human.tsv",sep="\t", header= TRUE))
...
```{r,fig.width=5, fig.asp=.8}
pc.genes<-gtf$external_gene_name[gtf$gene_biotype=="protein_coding"]
...
```{r,fig.width=5, fig.asp=.8}
pc.hist<-gtf$external_gene_name[gtf$gene_biotype=="protein_coding"]
...
# t-SNE by lincRNA
```{r,fig.width=5, fig.asp=.8}
linc<-gtf$external_gene_name[gtf$gene_biotype=="lncRNA"] # changed to lncRNA
...
misc<-gtf$external_gene_name[gtf$gene_biotype=="misc_RNA"]
...
snc<-gtf$external_gene_name[grepl("snRNA|misc_RNA|Mt_tRNA|snoRNA|scaRNA",gtf$gene_biotype)]
...
```{r,fig.width=5, fig.asp=0.8}
# older Ensembl versions used HIST, newer versions should use H1,2,3,4 maybe HI?
# only H19 of the first 4, many HI so use for now
hist<-gtf$external_gene_name[grepl("^HI",gtf$external_gene_name)]
