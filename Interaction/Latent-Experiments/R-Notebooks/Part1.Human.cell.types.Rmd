...
---
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("C:/Users/willi/Documents/ProjectPaper/M5R/Interaction/Latent-Experiments/"))
```
...
##Load count matrix
```{r}
counts<-read.delim(file = "C:/Users/willi/Documents/ProjectPaper/M5R/Interaction/Latent-Experiments/Data/GSE151334_counts.human.tsv",sep="\t", header= TRUE)
```
##Load the gene-to-biotype matrix
```{r}
gtf<-read.delim(file = "C:/Users/willi/Documents/ProjectPaper/M5R/Interaction/Latent-Experiments/R-Notebooks/GRC/GRCh38.id.name.biotype.tsv", sep='\t',  header=T)
...
# Need to change "avg_logFC" to "avg_log2FC", likely version change
top10 <-counts.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
...
# logFC -> log2FC
top500 <-counts.markers %>% group_by(cluster) %>% top_n(n = 500, wt = avg_log2FC)
...
```{r,fig.width=6, fig.asp=.5, echo=F}
# logFC -> log2FC
top1000 <-counts.markers %>% group_by(cluster) %>% top_n(n = 1000, wt = avg_log2FC)
...
# Plot lncRNA markers
```{r,fig.width=6, fig.asp=.5, echo=F}
# changed lincRNA -> lncRNA
linc<-as.character(gtf$external_gene_name[gtf$gene_biotype=="lncRNA"])
...
# Plot snRNA markers
```{r,fig.width=6, fig.asp=.5, echo=F}
sn<-as.character(gtf$external_gene_name[gtf$gene_biotype=="snRNA"])
...
```{r}
library(anndataR)
counts_ad = as_AnnData(counts)
counts_ad
```
```{r}
write_h5ad(counts_ad, path = "anndata.h5ad")
```

# Save biotypes to check

```{r}
write.csv(gtf, file="biomaRt_biotypes.csv")
gtf
```
