...
```{r}
counts<-read.delim(file = "C:/Users/willi/Documents/ProjectPaper/M5R/Interaction/Latent-Experiments/Pseudotime-Investigation/Data/GSE151334_counts.mouse.tsv",sep="\t", header= TRUE, row.names=1)
#counts.sorted<-counts.sorted[!grepl("Hist", rownames(counts.sorted)),!grepl("plate10", colnames(counts.sorted))]
```

```{r}
gtf<-read.delim(file = "GRC/GRCm38.id.name.biotype.tsv", sep='\t',  header=T)
gtf<-gtf[match(rownames(counts),gtf$external_gene_name), ] # do not sort
```
...
```{r}
Sys.setenv(RETICULATE_PYTHON = "C:/Python311")
#reticulate::use_virtualenv("C:/Users/willi/Documents/ProjectPaper/M5R_venv")
...
counts <- FindNeighbors(counts, dims = 1:15, verbose = FALSE, reduction = "pca") # fixed typo with old argument 'reduction.type = "umap"' and instead cluster on PCA space
...
FeaturePlot(counts, reduction = "umap", features = c("Nanog", "Sox2","Otx2", "Sox4", "Afp","Bmp7","T","Srf", "S100a4", "Tubb1", "Pax2", "Pax6", "Nes","Nog", "Krt18","Sox17", "Gata6","Gata4","Fgf5", "Sox7","Tubb3","Myc", "Bmp2", "Mir370", "Mir666","Mc1r", "Flk1", "Desmin", "Acta2", "Fbn2"), pt.size = 1, ncol = 4, order=T)# sort.cell=T) depreciated
```
...
```{r,fig.width=8, fig.asp=1.5}
top20 <-counts.markers %>% group_by(cluster) %>% top_n(n = 20, wt = avg_log2FC) ##select and plot top20 markers based on logFC
...
FeaturePlot(counts, features = c("Nanog" ,"Pou5f1","T",
                                 ...
                                 "Pax6","Sox1", "Hes3", "Sox2"
                                 ), pt.size = 0.8, ncol = 4, order=T)# sort.cell=T)
```
...
FeaturePlot(counts, features = c("Mir370" ,"Mir298","Mir387",
                                 ...
                                 ), pt.size = 0.8, ncol = 4, order=T, cols = c.cols)+ NoLegend() # sort.cell=T
```
...
#    igraph::dfs(graph = graph, root = root, mode = mode, ...)
#  }, 
#  ns = "igraph"
#)
```
...
library(dyno)
library(tidyverse)
dyno.expression<-GetAssayData(object = counts, layer="data")# slot="data")
dyno.expression<-Matrix::t(dyno.expression)

dyno.counts<- GetAssayData(object = counts, layer="data")# slot="counts")
...
```{r, fig.width=6, fig.asp=.6}
library(viridis)
plasma <- viridis(900, direction = 1, option = "D")
...
```{r}
counts_load <- readRDS("Mouse.count.Seurat.obj.rds")
```

```{r}
library(anndataR)
counts_ad = as_AnnData(counts_load)
counts_ad
```

```{r}
write_h5ad(counts_ad, path = "mouse.anndata.h5ad")
```
